...
def plot_avg_scores_over_time(
    ...
    # Axes Y (étirement conservé tel quel)
...
def plot_score_distribution_over_time(
    ...
    """
    Plot indépendant : moyenne (ligne + points) + bande min–max.
    Étirement Y identique au plot des moyennes.
    """
    if dist_df.empty:
        return
    ...
    for s, g in dist_df.groupby(by):
        ...
        # Filtrer les points valides pour la bande (évite les NaN finaux du snapshot)
        ...
        if np.any(valid_band):
            ...
            # Traits fins min & max (pour lecture précise)
            ax.step(xs_plot[valid_band], vmin[valid_band], where="post",
                    linewidth=0.6, linestyle="--", color=color, alpha=0.7)
            ax.step(xs_plot[valid_band], vmax[valid_band], where="post",
                    linewidth=0.6, linestyle="--", color=color, alpha=0.7)

    # Axes Y (étirement identique à ton plot existant)
    BETA = 0.4
    MARGIN = 0.05
...
def compute_score_distribution_over_time(
    ...
    À chaque t, on agrège sur les instances **couvrantes** pour chaque solver.
    """
    if segments_df.empty:
    ...
    # solver -> instance -> DF (segments ordonnés)
    per_solver: Dict[str, Dict[str, pd.DataFrame]] = {}
    for s in solvers:
        g_s = seg[seg["solver"].astype(str) == s]
        per_solver[s] = {
            inst: g_si.sort_values(["t_start", "t_end"]).reset_index(drop=True)
            for inst, g_si in g_s.groupby("basename")
        }
    ...
    for s in solvers:
        ...
        for k in range(len(T) - 1):
            ...
            if vals:
                ...
                m = vmin = vmax = float("nan")
                n = 0

            rows.append({
                "t": t0, by: s,
                "mean": m, "min": vmin, "max": vmax, "n_instances": n
            })

        # snapshot final t_N pour stabilité visuelle (reprend last_mean seulement)
        tN = T[-1]
        rows.append({
            "t": tN, by: s,
            "mean": (last_mean if last_mean is not None else float("nan")),
            "min": float("nan"), "max": float("nan"), "n_instances": None
        })
...
def generate_final_score_summary(
    ...
    """
    Pipeline: segments → moyennes + distribution → CSV+PNG.
    """
    seg = collect_instance_segments(df_traj, by=by, t_min=t_min, t_max=t_max)

    # 1) Moyennes (plot existant inchangé)
    ...
    # 2) Distribution (nouveau plot indépendant)
